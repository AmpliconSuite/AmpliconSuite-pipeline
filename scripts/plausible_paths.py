	...
	with open(graphf) as infile:
		...
		for line in infile:
			if line.startswith("sequence"):
				...
				#if ldir == "+" and rdir == "-":
				if skip_short_jumps and fields[0] == "discordant" and rchrom == lchrom and abs(rpos - lpos) < 800 and rdir != ldir:
	...
	with open(ofname,'w') as outfile:
		...
		for pind, p in enumerate(paths):
			if p:
				...
				outfile.write("Cycle=" + str(pind+1) + ";Copy_count=" + str(scaling_factor) + ";ProportionAmplifiedExplained=" + pweights[pind] + ";Segments=" + ",".join(fmtP) + "\n")


parser = argparse.ArgumentParser(description="Attempt to identify a longest path in the breakpoint graph consistent with CN ratios")
...
if args.scaling_factor:
	...
	scaling_factor = np.median(list(raw_cn.values()))
