...
- `--samtools_path`: Path to a specific samtools binary for use (e.g., /path/to/my/samtools). Or leave this unset to use system default. When samtools is installed in a user directory or in virtual environment, implementing samtools via PBS may cause loading shared libraries error (e.g., libcrypto.so.1.0.0).
